...
void Airport::initializeTracks(const QJsonValue& tracksValue)
{
    ...
    foreach (const QJsonValue &value, track_list)
    {
        ...
        if (!obj.value("id").isDouble())
            throw std::invalid_argument("Bad conf - Track id is invalid");
...
void Airport::initializeAgents(const QJsonValue& controlValue)
{
    ...
    foreach (const QJsonValue &value, control_agents)
    {
        ...
        if (!obj.contains("name"))
            throw std::invalid_argument("Bad conf - Control agent name is invalid");
        if (!obj.contains("id"))
            throw std::invalid_argument("Bad conf - Control agent id is invalid");
        if (!obj.contains("capacity"))
            throw std::invalid_argument("Bad conf - Control agent capacity is invalid");
...
void Airport::initializeStaticData(const QJsonValue& staticDataValue)
{
    ...
    if (static_data.contains("SMALLTIME"))
    {
        ...
        if (time < 0)
            throw std::invalid_argument("Bad conf - Times on track should be positive");
...
void Airport::notify(std::shared_ptr<Action> action)
{
...
Airport::Airport(const std::string &configure_file) : m_stopped(false)
{
    ...
    if (!agents_value.isArray() || agents_value.toArray().size() == 0)
        throw std::invalid_argument("Bad conf - Control agents needs to be an array of size greater than 1");

    if (!tracks_value.isArray() || tracks_value.toArray().size() == 0)
        throw std::invalid_argument("Bad conf - Tracks needs to be an array of size greater than 1");

    if (!static_data_value.isObject())
        throw std::invalid_argument("Bad conf - Static field needs to be an object");
